...
\newcommand{\liftsquare}[9]{
	\begin{tikzcd}[ampersand replacement=\&]
	{#1}\ar[r, "#2"]\ar[d, "#4" left]\&{#3}\ar[d, "#5" right]\\
	{#6}\ar[r, "#7" above]\ar[ur, dashed, "#9" above]\&{#8}
	\end{tikzcd}
}
...
%% Randomness
\pgfmathdeclarerandomlist{thuses}{{Thus}{Therefore}{As such}{As we wished}{By this great fortune}{Alors}{Donc}{Following this thread}{Consequently}{Thusforth}{Rejoice}{This can only mean one thing...}}
...
\pgfmathdeclarerandomlist{wewins}{{we win}{it's over}{all is good}{victory has been achieved}{we've crossed the finish line}{we're happy campers}{we've done it}{that's a wrap}}
